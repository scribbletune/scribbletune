...
module.exports = function(Scribbletune) {
	...
	Generate.clip = function(params) {
		...
		var 
			ticks = params.ticks || defaultParams.ticks,
			notes = params.notes || defaultParams.notes,
			pattern = params.pattern || defaultParams.pattern,
			//using eval in the following line as a lazy way out to convert stringed fraction to decimal
			noteLength = eval(params.noteLength) || defaultParams.noteLength,
		...
		notes.map(function(el){
			if(el.match(/[abcdefg]#?[0-9]/g) === null) throw new Error(el + 'is not a valid note!');
		});
