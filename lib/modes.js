...
var Modes = {
	...
	get: function(rootNote, octave, mode, appendNextOctaveNote) {
		...
		if(rootNote.indexOf('abcdefg') < 0) {
			throw new Error('Wrong root note!');
		}
		...
		if(isNaN(octave)) {
			throw new Error('You must specify a number for octave');
		}


		//check if octave is in the range of 1 to 8
		if(octave < 1 || octave > 8) {
			throw new Error('Octave must be in the range of 1 to 8!');
		}
