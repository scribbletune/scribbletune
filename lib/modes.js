...
var Modes = {
	...
	//function to return the modes available
	//@todo: memoize the output of this for subsequent calls
	getAvailableModes: function(){
		var modes = [];

		for (var mode in this.pattern) {
			modes.push(mode);
		}
	...
	get: function(rootNote, octave, mode, appendNextOctaveNote) {
		...
		if('a#bc#d#ef#g#'.indexOf(rootNote) < 0) throw new Error('Invalid root note!');
		/*validate root note2 - must not accept # as the first char*/
		if(rootNote.indexOf('#') == 0 ) throw new Error('Invalid root note!');
		/*validate mode*/
		...
		/*
		 * Filter notes as per pattern and append octave # to each note in the array
		 */
		var modeArr = [];
		this.pattern[mode].forEach(function(noteOn, index){
			(rootNote != 'c' && chromaticFromRoot[index] == 'c') && (octave++);
			if(noteOn) modeArr.push(chromaticFromRoot[index] + octave);
		});

		/*
		 * A mode or scale generally ends in the next octave of the root note
		 * Do this only if not specified otherwise. 
		 * Note: For rootNotes that are not C, the octave will have already incremented by now.
		 */

		if(appendNextOctaveNote) modeArr.push(rootNote + ( rootNote == 'c' ? octave+1 : octave));
